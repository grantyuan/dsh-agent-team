...
- Every Member's model rounds now pass a response-shape guard: a round that ends with only thinking content, or with text that neither calls a tool nor ends with the 任务结束 completion marker, is treated as a faulty round and the Host steers a `continue` into the same turn so the Member keeps working — up to three consecutive nudges before standing down, resetting on any healthy round. The Member persona states the matching rule: a finished turn that ends in plain text closes with 任务结束 as its very last words.
...
- The Host's ten-minute supervision pass now watches every enabled Member instead of only Claims holders, and treats a running Agent with no durable session event for thirty minutes as hung beside the stopped and error-presence cases. Repeated failures — three `agent/error` turns in a row or three failures inside twenty minutes — no longer replace the Member on the spot: a supervision alert first asks the team to reset it, one pass later the Host performs the same fresh-context reset the sidebar menu offers, and only a Member still unhealthy after that is archived and replaced by a same-role `-N` generation carrying its private memory and Channels, which asks `@human` in Channel to reassign the abandoned work.
- New `team_supervise` model tool: `status` reports what the supervision pass reads about one Member (pass state, presence, failure diagnostic, failures in the last twenty minutes), and `reset` gives an abnormal Member the same fresh-context reset the Human menu performs — a healthy Member refuses, and the caller cannot reset itself.
- Context compaction survives a broken summarizer: when a scheduled or pressure-forced compaction fails on a real error, the Host retries it through the Host catalog's other configured LLM routes one by one — excluding the Member's current one, up to three attempts — before reporting the failure.
- A Member that stopped with an error gains an 分析错误 (Analyze error) menu action: it dispatches the durable `doctor` Member — created on first use — to a fresh session with the failed Member's context as research material, and the doctor reports its analysis to you.
