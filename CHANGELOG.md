...
- Agent create/edit model pickers open with the warmed catalog and revalidate behind it, so a slow Host read delays a refresh instead of the picker; a failed read renders a retryable error instead of endless loading.
...
- Member private-memory context now states its index usage (`X.X KiB / 16 KiB` with a percentage) on every injection, and the index budget grows from 8 KiB to 16 KiB.
...
- Channel and member references in message bodies now render the same way: channel chips name the channel and hop to it, member chips name the member and open their session for active members. Unknown references stay plain text, and citing someone never notifies them.
- The Task Thread header stops competing with the conversation it introduces: its sections separate by space instead of drawing a second rule under the band's own, which gives the conversation back 66px of a 960px viewport instead of letting the header fill the fold. The Claim list now uses all three of its columns and reads in two lines — who owns the Claim and its state on the first, the work itself on the second — with a finished Claim stepped back so live work leads, and every presence dot lined up under the `Claims` heading. Runtime risks read as one localized line each (what kind of problem, whose, and the Host's own first sentence) with the full diagnostic on hover, replacing the wrapped English paragraph that used to fill the band. The opener under the Task or Thread title is one line at every width, so a long opening message no longer makes the header taller than a short one — it truncates with the full text on hover.
- Member memory upkeep now has a stated rule and a place to learn it: the persona keeps one resident sentence (a bounded index held under ~8 KiB against the 16 KiB ceiling, one line per topic cluster, detail in the note that line names), a bundled on-demand `member-memory-manager` skill owns what earns a line, the three tiers, compaction and demotion, and a Member's first-run `memory.md` scaffold states the same four sections.
- Your own face reaches the compact avatar stacks: an Inbox row or a Channel Thread entry that names you now draws your profile picture and current display name, while every Agent keeps the shared hue-and-initial identity it always had — Agents have no picture to reuse, so only the one chip that is you can change. Bytes that do not decode fall back to your name's initial, and such a row's accessible name reads your display name instead of the raw `member:human` id.
- A hovered or current row stops ringing the faces and unread dots it carries with a dark halo: the ring that cuts a chip out of its surface now paints the row's translucent wash over the fill underneath instead of darkening an already-darkened surface twice, so a chip keeps the size and edge it has on every other row. The sidebar's unread dot also cuts out of the sidebar fill it actually sits on rather than the page background.
- An Inbox row led by your own picture stops being the odd one out: its leading cluster now centres on the identity line it leads instead of sharing a baseline a picture does not have, so that row is no longer 4px taller than the rest and no longer pushes its identity line down 4px. Agent-led rows keep their height.
- Your picture stops standing in for other people in a Channel feed: the feed handed your avatar to every message row, so an Agent's own message drew your face instead of its hue and initial. A message identity seat now draws the picture only on your own row — the one row the profile describes — whatever the surface passes down.
- Members can recall their own earlier context: two new tools search and read the Member's own Session lineage — the current generation plus every generation it rolled over from — for remembered work, decisions, evidence, failures, files, or exact phrases, so nothing a rollover left behind becomes unreachable. Returning a hit is still the Member's own deliberate choice through `context_rollover`, the tools never conclude the turn, and a Session outside that lineage is never searched or read — historical transcript stays evidence, never authority.
- The context timeline is now read by that same engine across the Member's whole Session lineage, and every row carries a short `anchor` id: two boundaries that share a label and a token price — two Thread deliveries resolved in one turn — used to read as one duplicated row, and only a restorable row ever prints a full ref to return to. Which boundaries are offered is unchanged: a Team boundary still needs the retained prefix through it to stay inside one Thread.
