...
- Context continuity follows on `@wowyuarm/dsh-context-continuity` `^0.1.4`, which writes that producer-owned kind and reads both it and the renamed history; the Client follows rc.1's renamed icon exports, and the committed UI snapshot is refreshed with them.
- The `My profile` form now derives from the Team Host row's own volatile Config, the way rc.1 derives a plugin's form from its schema, and its three writes go through the Team's own `setHumanProfile` Remote, which keeps the ledger name-collision check that rc.1 leaves no plugin validate hook for.
- A fresh install or reinstall of published `0.1.14` is affected: it declares the engine as `^0.1.3`, so the registry resolves `0.1.4`, whose own peers name the `0.1.7` line. On the DSH `0.1.5` line that combination does not work — silently and as a whole: the bundle stops recognising the handoff, checkpoint continuation, and pressure notice the engine writes, and the engine stops reading tool results, so checkpoints and intents stop being recorded. An install already resolved to `0.1.3` is unaffected, and this release is the recovery: it declares `^0.1.4` and matches the engine's shapes on the certified line.
