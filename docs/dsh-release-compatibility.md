...
Release urgency from that record is resolved: npm `latest` has installed on this candidate line since Team `0.1.10` (2026-09-11), and `latest` is now Team `0.1.13`, published 2026-09-17, whose peers state the certified line `>=0.1.5-rc.1 <0.1.6` (the `0.1.9` line could neither admit nor run on this candidate).
