...
- Member supervision (`supervisor.ts`) is the automatic last resort behind `recovery.ts`. One pass every ten minutes — armed only after the startup Member restore settled, so cold Sessions never read as stops — watches every enabled Member. A Member reads settling while its Agent is live and a turn may move (a live-and-idle Member with open Claims waits by design and is never interrupted), and stopped when it is not active, carries an error presence, or has been running with no durable session event for thirty minutes. A stopped Member gets one bring-up attempt per pass, up to three.
- Repeated failures escalate instead of replacing on the spot: three `agent/error` turns with no clean turn between them, or three failures inside a twenty-minute rolling window, open a three-step chain advancing one step per pass. Step one posts a supervision alert to the Member's Channels, so any fellow Member may force the reset itself through `team_supervise`. Step two is the Host's own coercion — the same fresh-context reset the Human menu performs. Step three is the archive-and-replace handover below. Reading healthy again drops the chain and re-arms the gentle first step.
- Every completed model round passes the response-shape guard (`response-guard.ts`): a round that ends the turn with no tool call and no visible text — or whose text does not end with the `任务结束` completion marker — is a faulty round, and the Host steers one `continue` notice into the same turn so the Member keeps working. At most three consecutive nudges are spent before standing down, and any healthy round (a tool call or a proper marker ending) resets the episode; an interrupted turn is a Human abort, never a fault, and the notice stays a pure reminder in the context projection.
- Replacement runs as ordinary Host lifecycle inside one serialized step: the failed Member goes through the same `archiveMember` (its Claims release with public `claims_released` Activities), and a same-role generation joins under Human authority — handle takes the next free `-N` generation of the role, the mutable role facts carry over, private memory (`memory.md` and `notes/`, deliberately not `skills/`) is copied in before the replacement's first activation, and the predecessor's Channel participation is re-granted.
- The replacement then posts one taskless handover notice naming each released Claim across every Channel it inherited, with explicit recipients reaching all live fellow Members including `@human`, so the team learns who replaced whom and that settings and private memory carried over, while the admin reassigns the work through ordinary Task routing. Counters and the watch list are process-local — a restart re-earns the budget, mirroring the pressure policy — while every durable result is a ledger operation, so a crash mid-handover replays as a normal ledger, never as a half-faked Team fact.
...
- Accepted-Task auto compaction is retired. Member context pressure is owned by the Host pressure policy (see Tools and preset): budgets derive from the live route's context window, one handoff-budget notice per generation, forced in-place compaction at the hard limit that releases a step only once the measured pressure is provably below the limit (fail closed otherwise, one forced compaction per recovery chain), and one bounded compact-and-retry for provider context-overflow. Pending/error bookkeeping is process-local; only compactions in a transaction add Session history.
