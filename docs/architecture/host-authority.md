...
- Member supervision (`supervisor.ts`) is the automatic last resort behind `recovery.ts`. One pass every ten minutes — armed only after the startup Member restore settled, so cold Sessions never read as stops — watches exactly the enabled Members still holding active Claims. A Member judged abnormally stopped (unreachable or an error presence; a live-and-idle Member with open Claims waits by design and is never interrupted) gets one bring-up attempt per pass up to three; three `agent/error` occurrences with no clean turn end between them replace it on the spot.
