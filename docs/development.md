...
4. Link the Harness workspace, its vendor packages, and the sibling context-continuity engine into this repository's `node_modules` with `node scripts/link-harness-packages.mjs`, then build the bundle with `npm run build`. The engine must exist as the sibling `../dsh-context-continuity` and be built there (`npm run build`) first: the bundle imports `@wowyuarm/dsh-context-continuity` by name, and until the engine is published that link is what resolves the name; `DSH_CONTEXT_CONTINUITY_DIR` points the resolution at another checkout. Host tests resolve preset rows and the bundle's own unpublished rows (for example `@wowyuarm/dsh-agent-team/member-context`) through real `node_modules` lookups from this repository root, exactly as a profile install of the published bundle would.
...
| `DSH_CONTEXT_CONTINUITY_DIR` | isolated-checkout runs only | Points the engine resolution at another `dsh-context-continuity` checkout; unset for everyday work — the sibling name is the contract |
...
**The engine pointer works the same way.** `scripts/continuity-dir.mjs` owns the sibling `dsh-context-continuity` checkout that `link-harness-packages.mjs` links and `generate-typert.mjs` provisions into its analysis package, with `DSH_CONTEXT_CONTINUITY_DIR` as the only escape and the same fail-fast message. The engine is a separate repository and is not published yet, so that sibling link is the only thing resolving `@wowyuarm/dsh-context-continuity` today; the root manifest already declares it as a peer, which is what the boot-critical closure gate in `shipping.spec.ts` requires of every runtime root.

> TODO: the engine's install-time contract is still open — whether it is published to npm as a peer the profile install resolves, or reaches consumers some other way. Until it is settled, CI cannot run this repository's gate (it clones only the Harness checkout, and the engine is neither published nor clonable), so the gate runs only where the sibling engine checkout exists.
