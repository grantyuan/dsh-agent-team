...
触发器沿用创建表单里单选字段的带框形态（`.workspaceTrigger`，8px 圆角、34px 行高、`aria-haspopup="menu"` 加 `aria-expanded`），使这行「你在哪」不被读成频道列表的第一项；它常驻 business 色文件夹图标（因为它始终在展示一个选择），以自身文本写出所选 Workspace、完整路径挂 `title`，点击打开公共 `Menu` 并勾选当前项。
