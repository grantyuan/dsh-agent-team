...
- **Set up the environment contract.** Development and verification need the adjacent Harness checkout at `../deepseek-harness`, and installs go through `corepack pnpm` — never `npm install`, which breaks the workspace symlinks. Setup, generated artifacts, and the certified-tag rule: [`docs/development.md`](docs/development.md).
- **Agree on boundary changes first.** Host authority, the Team ledger, the model-facing tool surface, and Client slot wiring are guarded by [`AGENTS.md`](AGENTS.md) and [`docs/architecture.md`](docs/architecture.md). An issue that settles the shape up front is cheaper than a rewritten pull request.
...
`npm test` runs the generated-artifact, documentation, core-skill, and package-boundary checks before Vitest. The complete ladder — `lint` and `duplication` included — is [`docs/development.md`](docs/development.md) § "Verification gradient".
...
- **Evidence for visible changes**: screenshots at desktop and 390×844, plus keyboard/focus and dialog/menu states, per [`docs/frontend-design.md`](docs/frontend-design.md).
